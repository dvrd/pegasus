...
test_empty_op_begin_text :: proc(t: ^testing.T) {
	// At beginning of text (r1=-1), before a word char.
	// Result is 2 (.BeginText) after XOR removes NoWordBoundary.
	op := empty_op_context(-1, 'a')
	testing.expect(t, op == .BeginText, fmt.tprintf("expected BeginText, got %d", int(op)))
	// The VM would check: int(op & .BeginText) != 0 → satisfied
	testing.expect(t, satisfied(op, .BeginText), "BeginText should be satisfied")
}

@(test)
test_empty_op_end_text :: proc(t: ^testing.T) {
	// At end of text (r2=-1), after a word char.
	// r1='a' is word, boundary=1; r2=-1 → op |= EndText|EndLine → 5|3|1=7;
	// boundary stays 1 → XOR → 7^5=2 (.BeginText)
	op := empty_op_context('a', -1)
	testing.expect(t, int(op) == 2, fmt.tprintf("expected int(op)=2, got %d", int(op)))
	// The VM would check EndText(3) & 2 = 2 != 0 → satisfied
	testing.expect(t, satisfied(op, .EndText), "EndText should be satisfied via bitwise AND")
}

@(test)
test_empty_op_both_sentinels :: proc(t: ^testing.T) {
	// Empty text: both r1=-1 and r2=-1.
	// op = 5 | 2 | 0 | 3 | 1 = 7, boundary=0, no XOR.
	op := empty_op_context(-1, -1)
	testing.expect(t, int(op) == 7, fmt.tprintf("expected int(op)=7, got %d", int(op)))
	// All low-bit flags should be satisfied via bitwise AND.
	testing.expect(t, satisfied(op, .BeginText), "BeginText should be satisfied")
...
test_empty_op_word_boundary :: proc(t: ^testing.T) {
	// Transition from non-word to word char.
	// op=5, no r1 case matches, r2='a' → boundary ~= 1 → 1, XOR → 5^5=0.
	op := empty_op_context(' ', 'a')
	testing.expect(t, int(op) == 0, fmt.tprintf("expected int(op)=0, got %d", int(op)))
}

@(test)
test_empty_op_no_word_boundary :: proc(t: ^testing.T) {
	// Both word chars = no word boundary.
	// op=5, r1='a' → boundary=1, r2='b' → boundary ~= 1 → 0, no XOR.
	op := empty_op_context('a', 'b')
	testing.expect(t, op == .NoWordBoundary, fmt.tprintf("expected NoWordBoundary(5), got %d", int(op)))
}

@(test)
test_empty_op_newline_begin_line :: proc(t: ^testing.T) {
	// After newline, before word char.
	// op=5, r1='\n' → op |= .BeginLine(0) → 5, r2='a' → boundary ~= 1 → 1,
	// XOR → 5^5=0 (.BeginLine).
	op := empty_op_context('\n', 'a')
	testing.expect(t, op == .BeginLine, fmt.tprintf("expected BeginLine(0), got %d", int(op)))
}

@(test)
test_empty_op_before_newline :: proc(t: ^testing.T) {
	// Before newline, after word char.
	// op=5, r1='a' → boundary=1, r2='\n' → op |= .EndLine(1) → 5,
	// boundary stays 1, XOR → 5^5=0.
	op := empty_op_context('a', '\n')
	testing.expect(t, int(op) == 0, fmt.tprintf("expected int(op)=0, got %d", int(op)))
}
...
test_empty_op_word_boundary_at_text_start :: proc(t: ^testing.T) {
	...
	op := empty_op_context(-1, 'a')
	testing.expect(t, op == .BeginText, fmt.tprintf("expected BeginText(2), got %d", int(op)))
}

@(test)
test_empty_op_word_boundary_at_text_end :: proc(t: ^testing.T) {
	// After a word char at end of text.
...
test_empty_op_word_to_non_word :: proc(t: ^testing.T) {
	// Word char followed by non-word = word boundary.
	// op=5, r1='a' → boundary=1, r2=' ' → no case, boundary stays 1,
	// XOR → 5^5=0.
	op := empty_op_context('a', ' ')
	testing.expect(t, int(op) == 0, fmt.tprintf("expected int(op)=0, got %d", int(op)))
}
